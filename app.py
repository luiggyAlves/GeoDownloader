...
# ── Proteção por senha (defina ACCESS_PASSWORD no Railway → Variables) ────────
ACCESS_PASSWORD = os.environ.get("ACCESS_PASSWORD", "")

def check_auth(password):
    return not ACCESS_PASSWORD or password == ACCESS_PASSWORD
...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        auth = request.cookies.get("geo_auth")
...
<title>GEO Downloader — Acesso</title>
<link href="https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&family=DM+Sans:wght@300;400;500&display=swap" rel="stylesheet"/>
<style>
*{box-sizing:border-box;margin:0;padding:0}
:root{--bg:#0b0f14;--surf:#111820;--border:#1e2d3d;--accent:#00c8ff;--green:#00ff9d;
  --err:#ff5f5f;--text:#c8d8e8;--dim:#4a6278;--mono:'Space Mono',monospace;--sans:'DM Sans',sans-serif}
...
.box{position:relative;z-index:1;width:100%;max-width:380px;padding:0 20px;text-align:center}
.icon{font-size:36px;margin-bottom:20px}
h2{font-family:var(--mono);font-size:20px;color:#fff;margin-bottom:8px}
p{font-size:13px;color:var(--dim);margin-bottom:28px}
input{width:100%;background:#0b0f14;border:1px solid var(--border);border-radius:4px;
  color:#fff;font-family:var(--mono);font-size:14px;padding:12px 14px;
  outline:none;transition:border-color .2s;letter-spacing:.06em;text-align:center;margin-bottom:12px}
input:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(0,200,255,.1)}
input::placeholder{color:var(--dim);letter-spacing:.04em}
button{width:100%;background:var(--accent);color:#000;border:none;border-radius:4px;
...
.err{color:var(--err);font-size:12px;margin-top:10px;font-family:var(--mono);display:none}
...
<div class="box">
  <div class="icon">🔬</div>
  <h2>GEO Downloader</h2>
  <p>Acesso restrito ao grupo de pesquisa</p>
  <input id="pw" type="password" placeholder="Senha de acesso" onkeydown="if(event.key==='Enter')login()"/>
...
async function login(){
  ...
  else{ document.getElementById('err').classList.add('show'); document.getElementById('pw').value=''; }
}
</script>
</body></html>"""
...
def auth():
    ...
    if check_auth(data.get("password", "")):
        resp = jsonify({"ok": True})
        resp.set_cookie("geo_auth", ACCESS_PASSWORD, max_age=60*60*24*30, httponly=True, samesite="Lax")
        return resp
